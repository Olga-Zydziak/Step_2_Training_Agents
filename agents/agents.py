...
class CausalDiscoveryAgent(WorkerAgent):
    ...
    def _extract_python_code(self, text: str) -> str:
        """
        Niezawodnie wyodrębnia kod Pythona z odpowiedzi LLM,
        nawet jeśli jest on otoczony dodatkowym tekstem lub markdownem.
        """
        # Szukaj kodu wewnątrz bloku ```python ... ```
        match = re.search(r"```python\n(.*?)\n```", text, re.DOTALL)
        if match:
            return match.group(1).strip()
        # Jeśli nie ma bloku, zwróć cały tekst, usuwając białe znaki
        return text.strip()
...
"""
            ...
            for attempt in range(2):
                try:
                    ...
                    raw_response = self.llm.invoke(fix_code_prompt).content
                
                    # === NOWA, KLUCZOWA POPRAWKA ===
                    # Zamiast ślepo ufać odpowiedzi, filtrujemy ją, aby uzyskać czysty kod.
                    corrected_code = self._extract_python_code(raw_response)
                    print("  [SELF-HEAL] Wyodrębniono następujący poprawiony kod:\n" + corrected_code)
